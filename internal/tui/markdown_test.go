...
var testStyles = NewStyles(themes.DefaultTheme())

func TestRenderMarkdown_Bold(t *testing.T) {
	s := NewStyles(themes.DefaultTheme())
	got := RenderMarkdown("**bold text** here", 80, s)
...
func TestRenderMarkdown_Italic(t *testing.T) {
	s := NewStyles(themes.DefaultTheme())
	got := RenderMarkdown("*italic text* here", 80, s)
...
func TestRenderMarkdown_InlineCode(t *testing.T) {
	s := NewStyles(themes.DefaultTheme())
	got := RenderMarkdown("use `fmt.Println()` to log", 80, s)
...
func TestRenderMarkdown_Strikethrough(t *testing.T) {
	s := NewStyles(themes.DefaultTheme())
	got := RenderMarkdown("~~old text~~ new", 80, s)
...
func TestRenderMarkdown_Link(t *testing.T) {
	s := NewStyles(themes.DefaultTheme())
	got := RenderMarkdown("check [the docs](https://example.com) now", 80, s)
...
func TestRenderMarkdown_Image(t *testing.T) {
	got := RenderMarkdown("see ![diagram](img.png) here", 80, testStyles)
	stripped := stripANSI(got)
...
func TestRenderMarkdown_HeadingLevels(t *testing.T) {
	...
	for _, tc := range tests {
		got := RenderMarkdown(tc.input, 80, testStyles)
		stripped := stripANSI(got)
...
func TestRenderMarkdown_HTMLTags(t *testing.T) {
	got := RenderMarkdown("<strong>bold</strong> and <em>italic</em> and <code>code</code>", 80, testStyles)
	stripped := stripANSI(got)
...
func TestRenderMarkdown_StripsOtherHTML(t *testing.T) {
	got := RenderMarkdown("<div>hello <span>world</span></div>", 80, testStyles)
	stripped := stripANSI(got)
...
func TestRenderMarkdown_MixedBoldItalic(t *testing.T) {
	got := RenderMarkdown("**bold and *italic* together**", 80, testStyles)
	stripped := stripANSI(got)
...
func TestRenderMarkdown_EmptyString(t *testing.T) {
	got := RenderMarkdown("", 80, testStyles)
	if got != "" {
...
func TestRenderMarkdown_WidthWrapping(t *testing.T) {
	got := RenderMarkdown("this is a very long paragraph that should wrap at the specified width", 30, testStyles)
	stripped := stripANSI(got)
...
func TestRenderMarkdown_KeywordBold(t *testing.T) {
	got := RenderMarkdown("this is __also bold__ text", 80, testStyles)
	stripped := stripANSI(got)
...
func TestRenderMarkdown_UnderscoreItalic(t *testing.T) {
	got := RenderMarkdown("this is _italic_ text", 80, testStyles)
	stripped := stripANSI(got)
...
func TestCollectText(t *testing.T) {
	...
	output := RenderMarkdown("hello **world**", 80, testStyles)
...
func TestRenderTableTopBorder(t *testing.T) {
	got := renderTableTopBorder([]int{10, 20}, testStyles)
...
func TestRenderTableSepBorder(t *testing.T) {
	got := renderTableSepBorder([]int{10, 20}, testStyles)
	if !strings.Contains(got, "├") || !strings.Contains(got, "┼") || !strings.Contains(got, "┤") {
...
func TestRenderTableBotBorder(t *testing.T) {
	got := renderTableBotBorder([]int{10, 20}, testStyles)
	if !strings.Contains(got, "└") || !strings.Contains(got, "┴") || !strings.Contains(got, "┘") {
...
func TestRenderTableFrame_Empty(t *testing.T) {
	got := renderTableFrame([]int{}, "┌", "┬", "┐", "─", testStyles)
	if got != "┌┐" {
